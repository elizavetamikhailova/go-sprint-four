...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	var heightInCm = height / cmInM
	var meanSpeedSquare = math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)
	return (walkingCaloriesWeightMultiplier*weight + (meanSpeedSquare/heightInCm)*walkingSpeedHeightMultiplier*weight) * duration * minInH
